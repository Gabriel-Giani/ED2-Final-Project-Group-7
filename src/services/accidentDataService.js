...
export const accidentDataService = {
  ...
  async processAccidentData(accidents) {
    ...
    try {
      // Group accidents by road name
      const roadGroups = new Map();
      const processedRoads = new Set(); // Track which roads we've already processed

      accidents.forEach((accident) => {
        if (!accident.latitude || !accident.longitude || !accident.onroadname)
          return;

        const roadName = accident.onroadname.trim().toUpperCase();
        if (!roadGroups.has(roadName)) {
          roadGroups.set(roadName, {
        ...
        const group = roadGroups.get(roadName);
        group.accidents.push({
      ...
      // Fetch road geometries from major-roads table
      const roadSegments = [];
      const hotspots = [];
      const batchSize = 5; // Process roads in smaller batches
      let processedCount = 0;

      // Convert roadGroups to array for batch processing
      const roadEntries = Array.from(roadGroups.entries());

      while (processedCount < roadEntries.length) {
        const batch = roadEntries.slice(
          processedCount,
        ...
        for (const [roadName, group] of batch) {
          // Skip if already processed or too few accidents
          if (processedRoads.has(roadName) || group.count < 3) continue;
          processedRoads.add(roadName);

          // Query the major-roads table for matching road segments
          ...
          // Parse WKT and create road segments
          for (const road of result) {
            try {
              // Extract coordinates from WKT LINESTRING
              const wktCoords = road.WKT.replace("LINESTRING (", "")
                .replace(")", "")
                .split(",")
                .map((coord) => {
              ...
              // Find accidents near this road segment
              const nearbyAccidents = group.accidents.filter((accident) => {
                // Find minimum distance to any point on the road
                return wktCoords.some((coord) => {
                  const distance = Math.sqrt(
                    Math.pow(coord[0] - accident.longitude, 2) +
                      Math.pow(coord[1] - accident.latitude, 2)
                  );
                  return distance < 0.01; // Approximately 1km threshold
                });
              });

              if (nearbyAccidents.length > 0) {
                // Calculate intensity based on accident count
                const intensity = Math.min(1, nearbyAccidents.length / 20); // Normalize by 20 accidents

                // Create road segment
                roadSegments.push({
                  id: road.LINEARID,
                  name: road.FULLNAME,
                  coordinates: wktCoords,
                  count: nearbyAccidents.length,
                  intensity: intensity,
                });

                // Create hotspots along the road
                const segments = this.createRoadHotspots(
                  wktCoords,
                  nearbyAccidents
                );
                hotspots.push(...segments);
              }
            } catch (error) {
      ...
      console.error("Error in processAccidentData:", error);
      return { hotspots: [], roadSegments: [] };
    }
  },

  /**
   * Create hotspots along a road segment based on accident clusters
   * @param {Array} roadCoords - Array of road coordinates
   * @param {Array} accidents - Array of accidents near the road
   * @returns {Array} - Array of hotspot objects
   */
  createRoadHotspots(roadCoords, accidents) {
    const hotspots = [];
    const segmentLength = 0.01; // Approximately 1km segments
    ...
    for (let i = 0; i < roadCoords.length - 1; i++) {
      ...
      // Find accidents near this segment
      ...
      if (nearbyAccidents.length > 0) {
        ...
        // Calculate intensity based on accident count
        const intensity = Math.min(1, nearbyAccidents.length / 10);

        hotspots.push({
          id: `hotspot-${roadCoords[0][0]}-${roadCoords[0][1]}-${i}`,
          center: center,
          intensity: intensity,
          count: nearbyAccidents.length,
          road_name: nearbyAccidents[0].onroadname,
          county: nearbyAccidents[0].dotcounty
            ? this.getCountyNameFromCode(nearbyAccidents[0].dotcounty)
            : "Unknown",
          city: nearbyAccidents[0].townname || "Unknown",
        });
      }
    }
   ...
   */
  async supabaseQuery(table, searchColumn, searchValue) {
    try {
      const { data, error } = await supabase
        .from(table)
        .select("*")
